...
static int netem_change(struct Qdisc *sch, struct nlattr *opt)
{
	...
	if (tb[TCA_NETEM_SEED])
	{
		...
		if (nlaseed->reorder_corr_seed)
			q->reorder_cor.rng.seed = nlaseed->reorder_corr_seed;
		else
			q->reorder_cor.rng.seed = prandom_u32();

		if (nlaseed->duplication_corr_seed)
			q->dup_cor.rng.seed = nlaseed->duplication_corr_seed;
		else
