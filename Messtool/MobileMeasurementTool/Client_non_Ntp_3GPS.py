...
def logGPS(filepath, deviceNumber, killevent):
    with open(filepath + "_gps" + deviceNumber + ".txt", 'w') as output:
        call(["sudo", "python2", "gpsreader.py", "-d", "/dev/ttyUSB" + deviceNumber, "dump"], stdout=output)
...
if __name__ == "__main__":
    ...
    gpsthread = Thread(target=logGPS, args=(dirpath, 0,killevent))
    gpsthread = Thread(target=logGPS, args=(dirpath, 1,killevent))
    gpsthread = Thread(target=logGPS, args=(dirpath, 2,killevent))
